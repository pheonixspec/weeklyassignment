...
public class CollinearPoints {
    ...
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) {
            ...
            System.out.println("Are points collinear (by area): " + collinearArea);
            
            if (Math.abs(x2 - x1) > 1e-9 && Math.abs(x3 - x2) > 1e-9) {
                boolean collinearSlope = areCollinearBySlope(x1, y1, x2, y2, x3, y3);
                System.out.println("Are points collinear (by slope): " + collinearSlope);
            }
        }
    }
}
